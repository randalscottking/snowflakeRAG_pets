...
class SnowflakeRAGService:
    """Handles RAG operations using Snowflake Cortex within Snowflake"""
    ...
    def search_documents(self, query: str, search_service: str, limit: int = 5) -> List[Dict]:
        ...
        try:
            # Use Snowflake Cortex Search directly
            ...
            # Convert to list of dictionaries if results exist
            if search_results:
...
Please provide a helpful, informative response while maintaining appropriate medical disclaimers."""
            ...
            # Use Cortex Complete for response generation
            response = Complete(
                model=model,
                prompt=prompt,
...
def format_context_from_search(search_results: List[Dict]) -> tuple[str, List[Dict]]:
    ...
    for i, result in enumerate(search_results, 1):
        ...
        if isinstance(result, dict):
            content = result.get('content', result.get('text', result.get('chunk', str(result))))
            score = result.get('score', result.get('distance', 'N/A'))
            source = result.get('source', result.get('file_name', f'Document {i}'))
        else:
            content = str(result)
            score = 'N/A'
            source = f'Document {i}'
        ...
        with st.expander(f"📚 Sources Used ({len(sources)})", expanded=False):
            for source in sources:
                ...
                <div class="source-info">
                    <strong>Source {source['index']}</strong> - Score: {source['score']}<br>
